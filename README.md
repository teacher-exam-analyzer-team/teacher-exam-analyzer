...
| 시험지 생성 | FR-11 | 시험명, 설명, 시험 일자, 대상 반을 입력하여 시험지를 생성할 수 있어야 한다. |
...
|  | FR-15 | 시험지 미리보기는 학생 배부용 형식으로 문항과 빈 답안 칸을 보여주어야 한다. |
|  | FR-16 | 시험지를 저장하고 저장된 시험지 목록을 조회할 수 있어야 한다. |
|  | FR-17 | 저장된 시험지는 고유 ID 기준으로 삭제할 수 있어야 한다. |
|  | FR-18 | 시험지를 PDF로 출력할 수 있어야 한다. |
| 답안 입력 | FR-19 | 학생별 답안을 직접 입력하고 저장할 수 있어야 한다. |
|  | FR-20 | CSV 파일을 통해 여러 학생의 답안을 불러올 수 있어야 한다. |
|  | FR-21 | CSV의 학번 열을 기준으로 현재 반의 학생과 매칭해야 한다. |
|  | FR-22 | CSV는 UTF-8 BOM, CP949, EUC-KR 인코딩을 처리할 수 있어야 한다. |
|  | FR-23 | 답안 입력값 검증에서 미입력 문항, 학생 매칭 실패, CSV 형식 오류를 확인해야 한다. |
| 자동 채점 | FR-24 | 학생 답안을 정규화한 뒤 기준 정답 및 허용 답안과 비교해야 한다. |
...
| 결과 분석 | FR-29 | 반 평균 점수와 전체 정답률을 계산해야 한다. |
...
| 구분 | 테스트 ID | 테스트 대상 | 검증 내용 | 기대 결과 |
|---|---|---|---|---|
| 학생 관리 | TC-01 | 학생 등록 | 학생 이름, 학번, 반 정보를 입력하여 학생을 등록한다. | 학생 목록에 신규 학생이 표시된다. |
...
| 문제은행 관리 | TC-04 | 문제 등록 | 문제 내용, 유형, 난이도, 기준 정답을 입력하여 저장한다. | 문제 목록에 신규 문제가 표시된다. |
|  | TC-05 | 문제 검색/필터 | 유형, 난이도, 태그 조건으로 문제를 검색한다. | 조건에 맞는 문제만 목록에 표시된다. |
| 시험지 생성 | TC-06 | 시험지 자동 추출 | 유형별 문항 수와 난이도 조건을 입력하고 자동 추출을 실행한다. | 조건에 맞는 문제 수만큼 시험지에 담긴다. |
...
| 답안 입력 | TC-12 | 직접 답안 입력 | 학생별 답안을 직접 입력하고 저장한다. | `exam_id + student_id` 기준으로 답안이 저장된다. |
...
| 자동 채점 | TC-16 | 자동 채점 | 저장된 학생 답안을 기준 정답 및 허용 답안과 비교한다. | 정답 수, 오답 수, 점수, 정답률이 계산된다. |
...
| 결과 확인·분석 | TC-19 | 결과 보기 | 자동 채점 완료 후 결과 보기를 실행한다. | 학생별 요약과 문항별 상세 결과가 표시된다. |
...
| CSV·재채점·대시보드 확장 | TC-25 | CSV 형식 오류 | 잘못된 헤더나 형식의 CSV를 업로드한다. | CSV 형식 오류 메시지가 표시된다. |
